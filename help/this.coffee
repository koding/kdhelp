# 
# # This Help
#
module.exports = """
This is a Terminal, in your Browser. Think of it like a text interface to
your Virtual Machine, provided by Koding.

The virtual machine itself is running the Ubuntu operating system. It is a
real and normal Linux operating system, meaning you can run any Linux
program, library, and tool that you would expect. Real development in your
browser, with very few limitations.

To see what programs come with your VM by default, type "help programs" and
press return.

As always, additional help can be found on Koding University at:
  http://learn.koding.com

Or from the Koding community, at:
  https://koding.com/Activity
"""
